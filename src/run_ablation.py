...
CONFIG = {
    ...
    "batch_size"   : 16,
    "num_epochs"   : 5,       # 5 epochs sufficient for ablation comparison
    "freeze_clip"  : True,
    "fusion_lr"    : 3e-4,
    "clip_lr"      : 1e-5,
    "pos_weight"   : 1.5,
...
def train_and_evaluate(model, model_name, train_loader, val_loader):
    ...
    # Differential LR — small for CLIP, larger for fusion layers
...
def main():
    ...
    # ── 5 variants ordered simplest → most complex ────────────────
    variants = [
        (
            "1. Text Only",
        ...
        (
            "5. Cross-Attn + Static Reweighting (Phase 1)",   # ← updated label
            AdaptiveFusionModel(freeze_clip=CONFIG["freeze_clip"])
        ),
    ]
    ...
    print("\n")
    print("=" * 72)
    print(f"{'ABLATION STUDY RESULTS — Phase 1: Static Reweighting':^72}")
    print("=" * 72)
    print(f"{'Model':<47} {'AUROC':>7} {'Acc':>7} {'F1':>7} {'ΔAUROC':>8}")
    print("-" * 72)
    ...
    for r in results:
        ...
        print(f"{r['model']:<47} {r['auroc']:>7.4f} {r['acc']:>7.4f} "
    ...
    static_model_auroc  = results[4]["auroc"]
    reweighting_gain    = static_model_auroc - cross_attn_auroc

    print(f"\nKey finding (Phase 1):")
    print(f"  Cross-Attn only AUROC              : {cross_attn_auroc:.4f}")
    print(f"  Cross-Attn + Static Reweighting    : {static_model_auroc:.4f}")
    print(f"  Gain from static reweighting alone : {reweighting_gain:+.4f}")
    print(f"\n  → Phase 2 will replace static alpha with per-sample dynamic gating")
    print(f"     to push AUROC further.")

    # ── Save to File ──────────────────────────────────────────────
    save_path = os.path.join(CONFIG["results_dir"], "ablation_results_phase1.txt")
    with open(save_path, "w") as f:
        f.write("ABLATION STUDY RESULTS — Phase 1: Static Reweighting\n")
        f.write("=" * 72 + "\n")
        f.write(f"{'Model':<47} {'AUROC':>7} {'Acc':>7} {'F1':>7} {'ΔAUROC':>8}\n")
        f.write("-" * 72 + "\n")
        for r in results:
            delta     = r["auroc"] - baseline_auroc
            delta_str = f"+{delta:.4f}" if delta >= 0 else f"{delta:.4f}"
            f.write(f"{r['model']:<47} {r['auroc']:>7.4f} {r['acc']:>7.4f} "
                    f"{r['f1']:>7.4f} {delta_str:>8}\n")
        f.write("=" * 72 + "\n")
        f.write(f"\nGain from static reweighting : {reweighting_gain:+.4f}\n")
        f.write(f"Phase 2 (dynamic) to be run separately.\n")
